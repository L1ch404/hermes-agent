...
JOLINK_LOGO = r"""[bold #FFD700]       _       _     _       _[/]
...
JOLINK_HERO = """[bold #FFD700]joLink[/]
...
_UPSTREAM_REPO_URL = "https://github.com/L1ch404/hermes-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/l1ch404/hermes-agent"
...
_RELEASE_URL_BASE = "https://github.com/L1ch404/hermes-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical L1ch404/hermes-agent joLink repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"joLink v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else JOLINK_HERO
        _brand_name = _bskin.get_branding("agent_name", "joLink")
        _tagline = _bskin.get_branding("tagline", "Runtime evidence for coding agents")
    except Exception:
        _bskin = None
        _hero = JOLINK_HERO
        _brand_name = "joLink"
        _tagline = "Runtime evidence for coding agents"
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]{_brand_name}[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]{_brand_name}[/]")
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else JOLINK_LOGO
        console.print(_logo)
        console.print(f"[dim {dim}]    {_tagline}[/]")
        console.print()
